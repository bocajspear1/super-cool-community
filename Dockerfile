...
RUN apt-get -qq update && DEBIAN_FRONTEND="noninteractive" apt-get install -y apache2 libapache2-mod-php php-mysql mariadb-server

RUN /etc/init.d/mysql start && sleep 10 && apt-get -qq update && DEBIAN_FRONTEND="noninteractive" apt-get install -y phpmyadmin && /etc/init.d/mysql stop && sleep 10

COPY sccommunity.sql /tmp/sccommunity.sql
...
RUN /etc/init.d/mysql start && sleep 10; echo 'CREATE DATABASE `sccommunity`;' | mysql; mysql sccommunity < /tmp/sccommunity.sql
